//
// test program to read back MECOS chopper speed setpoint via CAN
//
...
for(j=0; j<200; j++)
{
...
#ifdef CAN_DEBUG
  printf("(%d)Sending request to MECOS via a REQ_MPDO message\n",j+1);
  ...
  if(nbytes != sizeof(frame))
    ...
    system("sudo ifconfig can0 down");
    if(canopen()!=0)
      perror("Error re-opening CAN interface");
    }
  ...
  while(dt<RX_TIMEOUT_SEC)
    ...
    if(nbytes > 0)
      ...
      if( ((frame.can_id&0x1FFFFFFF) == 0x2C0) &&
        ...
        printf("Current MECOS speed setpoint: %d Hz\n", setpoint_hz);
        break;
        }      
      }
